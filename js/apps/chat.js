...
var OSO_Chat = (function() {
    ...
    var MESSAGE_TOO_LARGE_MESSAGE = '这条消息有点长，我处理起来吃力。你分两条发我，好不好？';
    var IMAGE_TOO_LARGE_MESSAGE = '这张图片有点大，我这里一时处理不了。换一张小一点的再发我，好不好？';
    ...
    function open() {
        ...
        imageInput.addEventListener('change', function() {
            ...
            if (file.size > MAX_IMAGE_INPUT_BYTES) {
                clearSelectedImage();
                addBubble('agent', IMAGE_TOO_LARGE_MESSAGE);
                return;
            }
            compressImageFile(file).then(function(image) {
                ...
                clearSelectedImage();
                addBubble('agent', error && /large/.test(error.message || '')
        ...
        function sendMessage() {
            ...
            .catch(function(err) {
                ...
                addBubble('agent', err && (err.kind === 'model_offline' || err.name === 'AbortError')
                    ...
                    ? image ? IMAGE_TOO_LARGE_MESSAGE : MESSAGE_TOO_LARGE_MESSAGE
        ...
        function addBubble(role, text, image) {
            ...
            chatMessages.push({ role: role, text: text || (image ? '[图片]' : '') });
            debouncedSave();
